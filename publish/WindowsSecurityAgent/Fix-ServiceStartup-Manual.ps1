# Manual fix steps - Run PowerShell as Administrator first
# This script does NOT require #Requires -RunAsAdministrator
# but you MUST run PowerShell as Administrator before executing
...
if (-not $NoElevationCheck) {
    ...
    if (-not $isAdmin) {
        ...
        Write-Host ""
        Write-Host "Or use: Start-Process powershell -Verb RunAs -ArgumentList '-NoExit', '-File', '$PSCommandPath'" -ForegroundColor Cyan
        exit 1
    }
}
